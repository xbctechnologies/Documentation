...
PBFT 기반의 합의 알고리즘에서는 합의 과정에 참여하는 전체 노드 n 중에서 <img src="https://latex.codecogs.com/gif.latex?&#x5C;ge&#x5C;frac{2}{3}n"/> 가 동의 할 경우, 제안된 블록을 확정하고 블록체인의 다음 블록으로 연결 함으로써, 악의적인 노드 구성 비율이 <img src="https://latex.codecogs.com/gif.latex?&#x5C;frac{1}{3}"/> 을 넘지 않는 한도 내에서 어떠한 경우에도 합의에 도달 함을 보장한다.
...
3. 각 Replica 노드는 자신이 전파한 블록과 다른 노드로 부터 수신한 블록의 동일 여부를 확인한다. 동일한 블록을 전송한 노드의 수가 정족수 <img src="https://latex.codecogs.com/gif.latex?(&#x5C;frac{2}{3}n)"/> 를 넘었다면 해당 블록을 검증한다.
...
또한 전통적인 PBFT 에서는 모든 노드가 동일한 weight 를 갖지만, Tendermint 의 합의 알고리즘에서는 각 validator 가 예치한 지분 의 양에 비례하여 weight 를 갖기 때문에, 여기서 정족수란 validator 수가 아닌 validator 들이 예치한 지분(voting power) 총합의 <img src="https://latex.codecogs.com/gif.latex?&#x5C;frac{2}{3}"/> 를 의미한다.
...
각 Validator 들은 제안된 블록에 대하여 각자 검증후 전자 서명 한다. 서명된 블록은 다시 한번 전파되어 <img src="https://latex.codecogs.com/gif.latex?&#x5C;frac{2}{3}"/> 이상의 서명이 이루어진 블록에 대하여 합의된 것으로 간주한다.
...
|필드명|타입|설명|비고|
|---|---|---|---|
| ChainID | string | 현재 블록이 속한 X.Chain 에 대한 식별자. |  |
| ~~ChildChainID <sup>:small_blue_diamond:</sup>~~| ~~string~~ | ~~현재 블록이 X.Block 일 경우, 새로 생성될 X.Chain 의 식별자.~~ | ``XBlockInfo``.``NewChainID`` 로 이동. |
| Height | int64 | 현재 블록 높이. (= 블록 번호) |  |
| ~~Bindex <sup>:small_blue_diamond:</sup>~~| ~~int64~~ | ~~Parent Chain 과 전체 Child Chain List 에서 블록 순번.~~ | ``BlockExtension``.``BIndex`` 로 이동. |
| Time | Time | 블록이 생성된 로컬 시간. | Tenermint 의 Time 구조체 참조. |
| NumTxs | int64 | 블록에 포함된 Tx. 개수 |  |
| ~~Reward <sup>:small_blue_diamond:</sup>~~| ~~int64~~ | ~~현재 블록에 대한 보상량.~~ | ``BlockExtension``.``Reward`` 로 이동. |
| LastBlockID | BlockID |  |  |
| TotalTxs | int64 |  |  |
| LastCommitHash | byte[] |  |  |
| DataHash | byte[] |  |  |
...
|필드명|타입|설명|비고|
|---|---|---|---|
| ~~XBlockHash~~ | ~~byte[]~~ | ~~이전 X.Block 해쉬~~ | ``XBlockInfo``.``LastXBlockID`` 로 이동. |
| ~~Bindex~~ BIndex | int64 | Parent Chain 과 전체 Child Chain 에서 블록 순번. |  |
| Reward | ~~int64~~ BigNumber | 현재 블록에 대한 보상량. |  |
| Fee | ~~uint64~~ BigNumber | 현재 블록에 포함된 트랜잭션 수수료 총합. |  |
| GenesisInfo | ~~XGenesisBlock~~ XBlockInfo | 현재 블록이 X.Block 인 경우, X.Block 에 대한 정보를 기술. |  |
  
  
#### ~~``XGenesisBlock``~~ ``XBlockInfo`` <sup>:small_blue_diamond:</sup>
  
|필드명|타입|설명|비고|
|---|---|---|---|
| ~~XBlockHash~~ LastXBlockID | ~~byte[]~~ BlockID | ~~이전 X.Block 해쉬~~ 이전 X.Block |  |
| GenesisTime | string |  |  |
| ~~ChainID~~ | ~~string~~ |  | 아래 ``NewChainID`` 와 중복으로 삭제. |
| NewChainID | string | 현재 블록으로 부터 새로 확장될 X.Chain 의 식별자. |  |
| AirdropAssetHolders | ~~Map\<string, BigNumber\>~~ AssetHolder[] |  |  |
...
| ~~disconnectedCnt~~ | int32 | 연속저긍로 블록 합의에 빠진 블록수 |  |
...
| creator | byte[] | 데이터 생성 계정의 주소. | 업데이트 변경 불가. |
...
| ~~address~~ | ~~Address~~ | ~~``DataAccount`` 의 주소.~~ |  *``Transaction.receiver`` 를 사용.*|
| ~~data~~ | ~~Base64Encoded~~ | ~~데이터 원본. Base64 인코딩. RPC Interfaces 를 호출하는 시점에서만 사용되고, 이후 Mempool 저장 또는 네트워크 전파시에 이 필드는 사용하지 않는다.~~ | ~~``TxFilePayloadBody`` 를 포함 하고 있는 트랜잭션의  전자서명 값 ``Transaction.r`` 와 ``Transaction.s`` 계산시 이 필드는 제외 되어야 한다. ``Transaction.hash`` 역시 이 필드를 생략하고 계산되어져야 한다.~~ |
| dataHash | byte[] | 데이터 원본 ``data`` 의 해쉬값. ||
| dataSig | Base64Encoded | 데이터 원본  ``data`` 에 대한 생성자의 전자서명. | 생성자에 의하여 별도로 생성된 전자서명. |
...
|필드명|타입|설명|비고|
|---|---|---|---|
|amount | BigNumber | Validator 되기 위하여 Bonding 할 지분량. | xto 단위.|
...
|amount | BigNumber | 위임할 지분량. | xto 단위.|
...
|amount | BigNumber | 위임 취소 지분량. | xto 단위.|
...
|필드명|타입|설명|비고|
|---|---|---|---|
| RewardAmount | BigNumber | 블록합의에 참여한 Validator 들에게 주어지는 보상. | 1000e18 xto |
...
| MinBondingTxFee | BigNumber | TxBondingPayloadBody 를 포함하는 트랜잭션 최소 수수료. |  |
| MinGRProposalTxFee | BigNumber | TxGRProposalPayloadBody 를 포함하는 트랜잭션 최소 수수료. |  |
| MinGRVoteTxFee | BigNumber | TxGRVotePayloadBody 를 포함하는 트랜잭션 최소 수수료. |  |
| MinXTxFee | BigNumber | TxMakeXChainPayloadBody 를 포함하는 트랜잭션 최소 수수료. |  |
